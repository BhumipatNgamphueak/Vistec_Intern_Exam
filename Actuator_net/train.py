...
class Train():
    ...
    def eval_model(self):
        ...
        self.scaled_model_input = torch.cat(model_input,dim=0).cpu().detach().numpy()[:plot_length,:]
...
if __name__=="__main__":
    ...
    import os
    csv_path = "./app/resources/actuator_data.csv"
    if os.path.islink(csv_path):
        target = os.path.realpath(csv_path)
        dataset_name = os.path.basename(os.path.dirname(target))
        print("="*70)

...
class SingleEpisodeDataLogger(Node):
    ...
    def __init__(self, config_path: str, policy_path: str, policy_config: str,
                 task_id: int, episode_id: int, output_dir: str, passive_mode: bool = False):
        ...
        # Policy runner - always load (even in observe-only mode)
        if self.passive_mode:
            self.get_logger().info("⚡ OBSERVE-ONLY MODE: Will compute actions but not publish commands")
        ...
        self.computed_torques = np.zeros(12)  # Computed torques from policy
        ...
        # Subscribers - CRITICAL: Use correct QoS profiles!
        # Gazebo topics need BEST_EFFORT, orientation fixer output uses RELIABLE
        ...
        # Gazebo topic - needs BEST_EFFORT
        ...
        # Orientation fixer output - uses RELIABLE
        ...
        # Gazebo topic - needs BEST_EFFORT (backup for orientation)
        ...
        # Send initial standing pose until joint states arrive (skip in observe-only mode)
        if not self.passive_mode:
            ...
            # In observe-only mode, start logging immediately since external policy is controlling
        ...
        if not self.passive_mode:
            ...
            self.get_logger().info("Observe-only mode: Computing actions but letting external policy control robot")
    ...
    def control_loop(self):
        ...
        # Compute actions from policy (always, even in observe-only mode)
        ...
        # Store computed torques for metrics calculation
        self.computed_torques = torques.copy()

        # Publish joint commands (skip in observe-only mode to avoid conflict with external policy)
        if not self.passive_mode:
            for i, joint_name in enumerate(self.joint_names_sdk):
                if joint_name in self.joint_cmd_pubs:
                    msg = Float64()
    ...
    def collect_data_row(self, cmd_vel, action, loop_start_time):
        ...
        # Instantaneous power (use computed torques instead of measured efforts)
        instantaneous_power = np.sum(np.abs(self.computed_torques * self.joint_velocities))

        # Torque saturation count (Go2 motor limit ~40 Nm)
        torque_limit = 40.0  # Nm
        torque_saturation_count = np.sum(np.abs(self.computed_torques) > torque_limit)
        ...
        row['torque_saturation_count'] = int(torque_saturation_count)
...
def main(args=None):
    ...
    parser.add_argument('--policy', required=False, help='Path to policy checkpoint (.pt)')
    parser.add_argument('--policy_config', required=False, help='Policy configuration name')
    ...
    parser.add_argument('--passive', '--observe-only', action='store_true', dest='observe_only',
                        help='Observe-only mode: run policy for actions but don\'t publish joint commands (avoids conflicts with external policy)')
    ...
    logger = SingleEpisodeDataLogger(
        ...
        passive_mode=parsed_args.observe_only
    )
